...
for index, fulltext in enumerate(data['abstract']):
    text_vectors = []  # saves GloVe vectors for abstract (fulltext)
...
for index, fulltext in enumerate(data['abstract']):
    text_word_labels = []  # keep the labels of the all the words contained in a single abstract (fulltext) text

    # add labels for words in abstract (fulltext)
    for word in fulltext:
    ...
    # save labels for each abstract (fulltext)
