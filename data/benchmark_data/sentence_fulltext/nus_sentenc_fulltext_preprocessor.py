...
# remove punctuation
data['fulltext'] = data['fulltext'].apply(remove_punct)
data['keywords'] = data['keywords'].apply(keyword_remove_punct)
# print(data)
...
# tokenize text
data['fulltext'] = data['fulltext'].apply(tokenize_stem_lowercase)
print(data['fulltext'])
print('fulltext finish')
...
X = []
for fulltext in data['abstract']:
    text_vectors = []  # saves GloVe vectors for one title + abstract + fulltext

    # convert words in fulltext to GloVe vectors
    for word in fulltext:
        text_vectors.append(get_glove_vec(word))

    # save GloVe vectors for each title + abstract + fulltext
...
for index, fulltext in enumerate(data['abstract']):
    text_word_labels = []  # keep the labels of the all the words contained in a single title, abstract and fulltext

    # add labels for words in fulltext
    for word in fulltext:
        if word in data['keywords'][index]:  # the word is a keyphrase
    ...
    # save labels for each title + abstract + fulltext
